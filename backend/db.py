...
# Global thread lock to manage concurrent database access gracefully
db_lock = threading.Lock()

@contextmanager
def db_connection():
    """
    Thread-safe context manager for managing SQLite database connections.
    Enables Write-Ahead Logging (WAL) mode for high-concurrency read/write operations
    and handles transaction commit, rollback, and connection cleanup.
    """
    with db_lock:
